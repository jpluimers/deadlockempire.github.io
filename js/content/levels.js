var levels = {
    ...
    "S1-simple" : new Level(
        ...
        "<a href='https://msdn.microsoft.com/en-us/library/system.threading.semaphoreslim'>Semaphores</a> limit the number of threads that can access a resource at the same time. In C#, they are implemented by the SemaphoreSlim class." +
        ...
        "To take a coin, you can call the <code><a href='https://msdn.microsoft.com/en-us/library/dd270787'>" + LanguageDependentSemaphoreSlimWaitMethodName() + "()</a></code> method on the semaphore. If there are no coins on the stack, the method waits until someone returns a coin. If you don't want to wait forever, you can pass it how long should it wait, in milliseconds. In that case, <code>" + LanguageDependentSemaphoreSlimWaitMethodName() + "()</code> will return a boolean indicating whether it obtained a coin.<br>" +
        "The <code><a href='https://msdn.microsoft.com/en-us/library/dd235727'>Release()</a></code> method adds a coin on the stack. Normally, you would call <code>Release()</code> only after a <code>" + LanguageDependentSemaphoreSlimWaitMethodName() + "()</code> - you would take a coin, do something while you have it, and then give it back. However, you can also call <code>Release()</code> while you don't have any coins yourself. If you let Thread 1 run, you will see it do this: if it can't find a coin within 500 milliseconds, it will create a new one.<br><br>" +
