levels["H1-ManualResetEvent"] = new Level(
    ...
    "The <b>ManualResetEventSlim</b> (which supersedes the older ManualResetEvent that is maintained for backwards compatibility only) is an object with a single boolean flag and three methods - " + LanguageDependentManualResetEventSlimWaitMethodName() + ", " + LanguageDependentManualSetEventSlimWaitMethodName() + " and " + LanguageDependentManualResetEventSlimWaitMethodName() + ". The " + LanguageDependentManualResetEventSlimWaitMethodName() + "() method blocks <b>until</b> the \"reset event\" is \"signaled\" (it starts out nonsignaled). Whether the event is signaled or not is set manually by the programmer using the methods " + LanguageDependentManualSetEventSlimWaitMethodName() + "() and " + LanguageDependentManualResetEventSlimWaitMethodName() + "().<br><br>" +
    "For example, you might want to block a thread until some long computation finishes on another thread. To do this, you might create a ManualResetEvent named 'computationFinished' and call 'computationFinished." + LanguageDependentManualResetEventSlimWaitMethodName() + "()' on the first thread and 'computationFinished." + LanguageDependentManualSetEventSlimWaitMethodName() + "()' on the computing thread after the computation is complete.<br><br>" +
    "<small>BUGFIX (February 9, 2016): Previously, the <code>." + LanguageDependentManualResetEventSlimWaitMethodName() + "()</code> errorneously blocked when the reset event was signaled (the reverse of what it does in C#). This has been fixed.</small>",
    "<div class='story-outro'>Enemy neutralized. Let's see what's coming next.</div>" +
    "You've done well. Using ManualResetEventSlim is trickier if you use both " + LanguageDependentManualSetEventSlimWaitMethodName() + "() and " + LanguageDependentManualResetEventSlimWaitMethodName() + "() rather than only " + LanguageDependentManualSetEventSlimWaitMethodName() + "().",
    [
        new Thread([
            createOuterWhile(),
...
levels["H2-CountdownEvent"] = new Level(
    ...
    "Yes! When using the <a href='https://msdn.microsoft.com/en-us/library/system.threading.countdownevent'>CountdownEvent</a>, you must make extra sure that you are not leaving yourself open to deadlocks - the <i>." + LanguageDependentCountdownEventWaitMethodName() + "()</i> calls will block indefinitely if not enough <i>.Signal()</i> calls have been made. Suppose you use the CountdownEvent for loading data. If one of threads fails to load data and somehow crashes, therefore not signalling, the program will be blocked and you won't be able to terminate the waiting threads.",
    [
        new Thread([
...
levels["H4-Barrier"] = new Level(
    ...
    "<p>The <a href='https://msdn.microsoft.com/en-us/library/system.threading.barrier'>Barrier class</a> is quite safe when used correctly, though it must have been difficult to create correctly for the developers of the .NET framework. The Barrier has a fixed <i>number of participants</i> - in this case, <b>two</b>. It has only one useful method - <i>." + LanguageDependentSignalAndWaitMethodName() + "()</i> that blocks until all participants reach it. Then, all participant threads are let through the barrier and the barrier resets.</p>" +
    "Note that Delphi does not ship with a barrier implementation but you can use <a href='https://github.com/lordcrc/ThreadBarrier/blob/master/ThreadBarrier.pas'>unit ThreadBarrier.pas</a> as an alternative.",
